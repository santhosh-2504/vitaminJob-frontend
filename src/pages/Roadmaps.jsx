...
import { addStar, removeStar, downloadRoadmap, clearAllUserErrors } from '../store/slices/userSlice.js';
...
const RoadmapPage = () => {
  ...
  const isStarred = (roadmapId) => {
    return user?.roadmaps?.includes(roadmapId);
  };

  const toggleStar = (roadmapId) => {
    if (!user?._id) {
      toast.error("Please login to star roadmaps");
      navigate('/login');
      return;
    }

    if (isStarred(roadmapId)) {
      dispatch(removeStar(roadmapId));
    } else {
      dispatch(addStar(roadmapId));
    }
  };
  ...
  const handleDownload = (roadmapId, title) => {
    ...
    localStorage.setItem('roadmapsScrollPosition', window.scrollY.toString());
                    ...
                    >
                      <div className="p-4 pb-16">
                        ...
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <button
                            onClick={() => toggleStar(roadmap._id)}
                            className="text-gray-400 hover:text-yellow-500 transition-colors"
                          >
                            <Star
                              className={`w-5 h-5 ${
                                isStarred(roadmap._id)
